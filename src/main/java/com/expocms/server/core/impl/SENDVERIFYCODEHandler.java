...
@Service("SENDVERIFYCODEHandler")
@Transactional
public class SENDVERIFYCODEHandler extends AbsBaseHandler {
	...
	public BaseResponse handleRequest(IRequest request) {
		...
		String phone = ((SendVerifyCodeRequest)request).getPhoneNo();
		...
		try {
			...
			String smsContent = "" + msg + "（融道验证码），有效期5分钟，千万不要告诉别人哦！你正在使用融道找回密码。【融道理财】";
...
//			smsMsgEntity.setMessage("" + msg + "（融道验证码），有效期5分钟，千万不要告诉别人哦！你正在使用融道找回密码。【融道理财】");
//			
